...
public class Draw {
    ...
    private void drawConnection(node_metadata src, node_metadata dest, Point2D label_location) {
        ...
        StdDraw.text(label_location.x(), label_location.y()+9, g.getEdge(src.getKey(), dest.getKey()).getTag(), degree);// print text connection
    }
    ...
    public void remove_from_filter(String str) {
        if (relationship_to_filter.contains(str)) relationship_to_filter.remove(str);
    }
    ...
    private void drawNodes() {
         List<node_metadata> graphNode =  new ArrayList<> (g.getV());
